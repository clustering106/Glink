...
caffe.set_mode_cpu()
from utils.onnx2caffe.do_onnx_caffe._transformers import ConvAddFuser,ConstantsToInitializers
from utils.onnx2caffe.do_onnx_caffe._graph import Graph

import utils.onnx2caffe.do_onnx_caffe._operators as cvt
import utils.onnx2caffe.do_onnx_caffe._weightloader as wlr
from utils.onnx2caffe.do_onnx_caffe._error_utils import ErrorHandling
...
def do_convert(onnx_path, prototxt_path, caffemodel_path):
    ...
    net = convertToCaffe(graph, prototxt_path, caffemodel_path)
    print('output_names:', output_names)
    return net

# python utils/onnx2caffe/convertCaffe.py runs/train/exp/weights/best.onnx 
...
#     prototxt_path = sys.argv[2] if len(sys.argv) >= 3 else onnx_path.replace('.onnx', '.prototxt')
#     caffemodel_path = sys.argv[3] if len(sys.argv) >= 4 else onnx_path.replace('.onnx', '.caffemodel')
#     graph, output_names = getGraph(onnx_path)
